...
class Formula:
    ...
    def create_formula_from_string(self, string_formula):
        """
        Creates the formula from given string in the input.
    ...
    def assign_fresh_variables(self, variable=1, fresh_variables_dict=None):
        ...
        if self.root:
            ...
            # assigned to a fresh variable. If it has been then use this fresh variable, otherwise assign a new unused
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ltr", default=False, type=bool, help="Specify if the Tseitin encoding should use "
                                                                "equivalences (ltr=False) or only left-to-right "
                                                                "implications (ltr=True)")
    args = parser.parse_args()

    input_file = open(args.input, mode="r") if args.input else sys.stdin
    string_formula = input_file.read()
    ...
    formula.tseitin_encoding(left_to_right=args.ltr, output_file=args.output)
